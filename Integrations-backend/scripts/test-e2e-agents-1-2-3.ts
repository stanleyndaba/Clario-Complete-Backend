 ...
 * Tests the complete flow: OAuth → Data Sync → Claim Detection
 * 
...
async function setupTestUser() {
  logger.info('\n📋 Setting up test user (simulating Agent 1 OAuth)...');
...
async function checkAgent3DetectionResults() {
  logger.info('\n🔍 Checking Agent 3: Claim Detection Results...');
...
async function checkDetectionQueue() {
  logger.info('\n📋 Checking detection_queue status...');
...
async function testPrepareClaimsDirectly() {
  logger.info('\n🧪 Testing prepareClaimsFromNormalizedData directly...');
...
async function printSummary() {
  logger.info('\n' + '='.repeat(60));
  logger.info('📊 TEST SUMMARY: Agents 1-2-3 E2E Pipeline');
...
async function main() {
  logger.info('🚀 Starting Agents 1-2-3 E2E Test Pipeline');
  logger.info('='.repeat(60));

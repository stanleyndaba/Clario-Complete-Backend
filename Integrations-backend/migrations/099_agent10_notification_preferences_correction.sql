-- Migration: 099_agent10_notification_preferences_correction
-- Purpose: correct Agent 10 notification preference schema for environments that already ran an earlier 098.
...
BEGIN
  ...
  IF unresolved_count > 0 THEN
    RAISE EXCEPTION 'Notification preference correction failed: % rows still missing tenant_id', unresolved_count;
  END IF;
END $$;
...
COMMENT ON TABLE user_notification_preferences IS 'Tenant-scoped notification preference ownership for Agent 10.';
COMMENT ON COLUMN user_notification_preferences.preferences IS 'JSONB storing per-notification-type email and in-app toggles.';

...
export class TokenManager {
  ...
  async getTokenWithStatus(
    ...
    try {
      ...
      try {
        ...
        void this.migrateLegacyTokenRecordToCurrentEnvelope(userId, provider, tokenRecord, tokenData);
      } catch (error: any) {
  ...
  private shouldMigrateTokenRecordToCurrentEnvelope(tokenRecord: TokenRecord): boolean {
    const accessInspection = this.credentialCrypto.inspect({
      iv: (tokenRecord as any).access_token_iv,
      data: (tokenRecord as any).access_token_data
    });
    const refreshInspection = this.credentialCrypto.inspect({
      iv: (tokenRecord as any).refresh_token_iv,
      data: (tokenRecord as any).refresh_token_data
    });

    return accessInspection.result === 'valid_legacy_format' || refreshInspection.result === 'valid_legacy_format';
  }
  ...
  private async migrateLegacyTokenRecordToCurrentEnvelope(
    ...
    if (!this.shouldMigrateTokenRecordToCurrentEnvelope(tokenRecord)) {
      return;
    }
    ...
    try {
      await dbTokenManager.updateToken(
        userId,
        provider,
        this.encrypt(tokenData.accessToken),
        tokenData.refreshToken ? this.encrypt(tokenData.refreshToken) : undefined,
        tokenData.expiresAt,
        tokenRecord.tenant_id,
        tokenRecord.store_id
      );
      logger.info('Provider credential migrated to current envelope', {
        userId,
        provider,
        tokenId: tokenRecord.id
      });
    } catch (error: any) {
      logger.warn('Provider credential legacy migration deferred', {
        userId,
        provider,
        tokenId: tokenRecord.id,

...
const MANUAL_EVALUATION_AREA_DEFINITIONS: Array<{
...
}> = [
  {
    ...
    optionalSources: [],
    evidenceRequired: ['Inventory ledger event', 'SKU/FNSKU', 'quantity gap', 'valuation basis'],
  ...
  },
  {
    ...
    optionalSources: [],
    evidenceRequired: ['Order ID', 'refund/settlement row', 'return row when present', 'amount and quantity context'],
    manualUploadSupported: true,
  },
  {
    key: 'broken_goods',
    ...
    optionalSources: [],
    evidenceRequired: ['Inventory ledger event', 'reason/disposition', 'SKU/FNSKU', 'quantity movement'],
    manualUploadSupported: true,
  },
  {
    key: 'fee_phantom',
    ...
    optionalSources: [],
    evidenceRequired: ['Fee event', 'fee type', 'charged amount', 'expected fee basis or sign expectation'],
    manualUploadSupported: true,
  },
  {
    ...
    const truth = manualReport
      ? {
          ...
          message: manualReport.filesProcessed > 0
            ? `Margin processed ${manualReport.filesProcessed} uploaded report${manualReport.filesProcessed === 1 ? '' : 's'} across ${manualReport.sourceFamilies.length} supported source ${manualReport.sourceFamilies.length === 1 ? 'family' : 'families'}. Coverage is limited to the reports and periods provided. This is a partial examination: Margin evaluated what the supplied evidence can establish; missing or failed sources remain unknown, and no connected Amazon data was used for this audit.`

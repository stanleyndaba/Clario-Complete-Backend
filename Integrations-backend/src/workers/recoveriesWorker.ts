...
class RecoveriesWorker {
  ...
  async runRecoveriesForAllTenants(): Promise<RecoveryStats> {
    ...
    try {
      ...
      if (error) {
        logger.error('❌ [RECOVERIES] Failed to get approved cases', { error: error.message });
        stats.errors.push(`Failed to get cases: ${error.message}`);
        return stats;
      }

      if (!approvedCases || approvedCases.length === 0) {
        logger.debug('ℹ️ [RECOVERIES] No approved cases needing recovery detection');
        return stats;
      }

      logger.info(`📋 [RECOVERIES] Found ${approvedCases.length} approved cases needing recovery detection`);
      ...
      for (const [userId, userCases] of casesByUser) {
        try {
          // Update recovery status to 'detecting'

...
router.post('/start', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const { detectionResultId } = req.body || {};
    ...
    const candidate = { sellerId: userId, sku: result.sku, asin: result.asin, quantity: result.quantity, detectionDate: result.detected_at } as any;
    const validation = await evidenceValidatorService.validate(candidate);

    let dispute: any;
    if (validation.status === 'proof_found') {
      // Create dispute, link evidence, and enqueue filing
      dispute = await (enhancedDetectionService as any).createDisputeCase(result);
      await supabase
    ...
    if (validation.status === 'ambiguity') {
      // Create a smart prompt for quick user selection, do not block with email
      const promptId = await smartPromptService.createEvidenceSelectionPrompt(
        userId,
        null,
        `Select the correct invoice for SKU ${result.sku || result.asin}`,
        validation.options.map((o, idx) => ({ id: String(idx + 1), label: o.label, evidence_document_id: o.evidenceDocumentId }))
      );
    ...
    // No proof found: skip auto filing
...
router.get('/status/:id', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const prediction = await predictablePayoutService.estimate(id, userId);
    // Prefer persisted fields if present
    ...
    return res.json({ success: true, dispute: data, amountRecovered: data.resolution_amount || 0, paidDate: data.resolution_date || null, expectedAmount, expectedPaidDate, confidence });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: error?.message || 'Internal error' } });
  }
});

...
router.get('/upcoming-payments', wrap(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 1. Get ALL dispute_cases (filed claims) - any status
    const { data: disputeCases, error: disputeError } = await dbClient
      .from('dispute_cases')
    ...
    if (detectionError) {
      logger.warn('❌ [UPCOMING PAYMENTS] Failed to fetch detection results', { error: detectionError.message, userId });
    } else if (detectionResults && detectionResults.length > 0) {
      // Filter out detection results that already have a dispute case
      const existingClaimIds = new Set(allRecoveries.map(r => r.claim_id));

      detectionResults.forEach((dr: any) => {
        // Skip if already in dispute_cases
        if (existingClaimIds.has(dr.id)) return;

        allRecoveries.push({
          id: dr.id,
          claim_id: dr.id,
          type: dr.anomaly_type || 'detection',
          status: dr.status || 'Open',
          amount: parseFloat(dr.estimated_value?.toString() || '0'),
          currency: dr.currency || 'USD',
          ...
          // Map to frontend field names  
          guaranteedAmount: parseFloat(dr.estimated_value?.toString() || '0'),
    ...
    allRecoveries.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    logger.info('✅ [UPCOMING PAYMENTS] Found claims for pipeline', {
      userId,
      totalCount: allRecoveries.length,
      fromDisputeCases: disputeCases?.length || 0,
      fromDetectionResults: detectionResults?.length || 0
    });
    ...
    return res.json({
      ...
      breakdown: {
        dispute_cases: disputeCases?.length || 0,
        detection_results: detectionResults?.length || 0
      }
    });
